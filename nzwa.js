	...
	nzwa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nSeja bem vindo ao grupo!♥️\n*Não seja um membro ghost!😔🤙* *${mdata.subject}*`
				...
				teks = `Adeus cornin!🐂🐂 @${num.split('@')[0]}ðŸ‘‹`
				let buff = await getBuffer(ppimg)
				nzwa.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		nzwa.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				case 'demote':
					...
					if (!isGroupAdmins) return reply(botLangs.admin())
					if (!isBotGroupAdmins) return reply(botLangs.Badmin())
					...
					if (mentioned.length > 1) {
						...
						mentions(`Rebaixado com sucesso @${mentioned[0].split('@')[0]} Torne-se um membro do grupo...`)
						nzwa.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
              ...
              await nzwa.sendMessage(from, options, text)
               break
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo teks, contoh ${prefix}marvellogo Nazwa Canss`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(botLangs.wait())
               ...
               nzwa.updatePresence(from, Presence.composing) 
				...
				case 'ytmp3':
					if (args.length < 1) return reply('Urlnya mana um?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(botLangs.Iv())
				...
				/*case 'semoji':
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(botLangs.stick())
				...
				case 'tulis':
					...
					nzwa.sendMessage(from, buff, image, {quoted: mek, caption: botLangs.success()})
                                        await limitAdd(sender)
					break
	case 'kbbi':
            nzwa.updatePresence(from, Presence.composing) 
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return nzwa.sendMessage(from, 'Onde está o código do idioma?', text, {quoted: mek})
                                   if (!isRegister) return reply(mess.only.daftarB)
                                   if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(botLangs.ownerB())
					prefix = args[0]
				...
				case 'block':
					...
					nzwa.sendMessage(from, `Pedidos recebidos, bloquear ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					nzwa.updatePresence(from, Presence.composing) 
				...
				case 'send':
					...
					nzwa.sendMessage(número+'@s.whatsapp.net', mensagem de texto)
					break
					case 'quotesnime':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'alay':
					...
					nzwa.groupLeave (from) 
                     ...
                     setTimeout( () => {
					nzwa.updatePresence(from, Presence.composing) 
					nzwa.sendMessage(from, 'Tchau‘‹', text) // ur cods
					}, 0)
                     break
				...
				case 'chord':
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Qual nome da música?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (!isGroup) return reply(langss.group())
                                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isGroupAdmins) return reply(botLangs.Badmin())
					if (args.length < 1) return reply('ketik 1 untuk mengaktifkan')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('fitur sudah aktif')
						welkom.push(from)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('â¬ SUCSESS â­ ative o recurso de boas-vindas neste grupo')
                ...
                case 'truth':
					...
					break	
                case 'level':
                if (!isLevelingOn) return reply(botLangs.levelnoton())
                if (!isGroup) return reply(langss.group())
                const userLevel = getLevelingLevel(sender)
                const userXp = getLevelingXp(sender)
                if (userLevel === undefined && userXp === undefined) return reply(botLangs.levelnol())
            ...
            case 'leveling':
                ...
                if (args[0] === '1') {
                    ...
                    fs.writeFileSync('./database/json/leveling.json', JSON.stringify(_leveling))
                     reply(botLangs.levelon())
                } else if (args[0] === '0') {
                    _leveling.splice(groupId, 1)
                    fs.writeFileSync('./database/json/leveling.json', JSON.stringify(_leveling))
                     reply(botLangs.leveloff())
                } else {
				...
				if (!isRegister) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					...
					if (args.length < 1) return reply('Urlnya mana um?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(botLangs.Iv())
					reply(botLangs.wait())
				...
				case 'clone':
					...
					break
                                ...
                                case 'event':
                                        ...
                                        break
                                case 'register':
                                        if (isRegister) return  reply(`「 JÁ SE REGISTRE 」 \n\nKakak já registrado kak...`)
					...
					break*/
                                ...
                                case 'tagme':
                                        ...
                                        var nom = mek.participant
                                        const tagme = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} Essa é a minha marca!`,
                                                contextInfo: { mentionedJid: [nom] }
                                        }
                                ...
                                case 'happymod':
                                        toby = body.slice(10)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (args.length < 1) return reply(`que jogo você está procurando mana? \nContoh : ${prefix}happymod pubg`)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
